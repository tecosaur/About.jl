...
function vecbytes(io::IO, items::DenseVector{T};
    ...
    elseif hassizeof(T) && Base.allocatedinline(T)
        sizeof(T), if (isprimitivetype(T) || (isstructtype(T) && all(hassizeof, fieldtypes(T)))) && Base.array_subpadding(UInt8, T)
            reinterpret(UInt8, items)
        else
            unsafe_wrap(Vector{UInt8}, Ptr{UInt8}(pointer(items)), (sizeof(Ptr) * length(items)))
        end
    else
